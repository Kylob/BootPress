...
class Blog_auth extends CI_Driver {
  ...
  public function __construct () {
    ...
    if (empty($user)) { // insert
      ...
      foreach ($admin as $field => $value) {
        if ($user[$field] != $value) {
          $this->db->update('users', 'email', array(ADMIN_EMAIL => $admin));
          break;
        }
      }
    }
  ...
  public function info ($user_id) {
    ...
    $this->db->query('SELECT id, name, email, admin, approved, strftime("%s", registered) AS registered FROM users WHERE id IN(' . implode(', ', array_keys($users)) . ')');
  ...
  public function check ($email, $password='') {
    ...
    if (empty($check)) return $this->db->value('SELECT id FROM users WHERE email = ? AND password = ?', array($email, sha1($password)));
    $add = array_shift($check);
    return $this->db->value('SELECT id FROM users WHERE email = ? AND password = ? AND ' . $add, array($email, sha1($password)));
  }
  
  public function register ($name, $email, $password) {
    $new_user = ($user_id = $this->check($email)) ? false : true;
    if ($new_user) $user_id = $this->db->insert('users', array('name'=>$name, 'email'=>$email, 'password'=>sha1($password)));
  ...
  public function get_users_groups ($user_id) {
    ...
    $users = array();
    foreach ((array) $user_id as $id) $users[$id] = array();
    $this->db->query('SELECT u.user_id, g.name FROM user_groups AS u INNER JOIN groups AS g ON u.group_id = g.id WHERE u.user_id IN(' . implode(', ', array_keys($users)) . ') ORDER BY g.name ASC');
  ...
  public function get_groups_users ($group) {
    ...
    $this->db->query('SELECT u.user_id, u.group_id, g.name FROM user_groups AS u INNER JOIN groups AS g ON u.group_id = g.id WHERE u.user_id > 0 AND u.group_id IN (' . implode(', ', $this->group_id(array_keys($groups))) . ') ORDER BY u.user_id, u.group_id ASC');
  ...
  private function group_id ($name) {
    ...
    foreach ((array) $name as $group) {
      if (empty($group)) {
        ...
        $group_id = $this->db->value('SELECT id FROM groups WHERE name = ?', array($group));
        if (empty($group_id)) $group_id = $this->db->insert('groups', array('name'=>$group));
      }
...
/* End of file Blog_auth.php */
/* Location: ./application/libraries/Blog/drivers/Blog_auth.php */