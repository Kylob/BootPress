  ...
  public function display ($content='') {
    ...
    if (is_admin(2)) {
      ...
      if ($ci->blog->name != '') {
        ...
        $profiler = ($ci->session->native->userdata('profiler')) ? ' checked="checked" ' : ' ';
    ...
    if (isset($driver['users'])) {
      ...
      if (is_admin(1)) {
        $menu = array($user . $ci->session->userdata('name') => ($this->view == 'setup' ? '#' : array(
        ...
        $menu = array($user . $ci->session->userdata('name') => array(
          'Edit Your Profile' => $page->url($this->url, 'users'),
          'Logout' => $page->url($this->url, 'users/logout')
        ));
      } else {
    ...
    $links = '<ul class="sidebar-menu">' . $bp->links('li treeview', $links, array('active'=>$page->url($this->url, $this->view))) . '</ul>';
