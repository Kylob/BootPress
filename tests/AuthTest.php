...
class AuthTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testLoginAndLogoutMethods()
    {
        ...
        $this->assertNotEmpty($page->session->get('bootpress/cookie'));
        $this->assertEquals(1, $page->session->get('bootpress/verified'));
        ...
        $page->session->remove('bootpress/verified');
        $this->assertFalse($auth->isVerified());
        $auth->logout();
        $this->assertEmpty($page->session->all());
        $this->assertNull($page->session->get('bootpress/cookie'));
        $this->assertNull($page->session->get('bootpress/verified'));
    ...
    public function testConstructorLoggingInAndOut()
    {
        ...
        $auth = new Auth();
        $auth->login(1);
        $session = $page->session->get('bootpress/cookie');
        $this->assertNotEmpty($session);
        list($id, $series, $token) = explode(' ', $session);
        ...
        $session = $page->session->get('bootpress/cookie');
        list($id, $series, $token) = explode(' ', $session);
        $cookie = base64_encode(implode(' ', array($id, sha1($series), sha1($token)))); // a database mismatch
        ...
        $page->session->remove('bootpress/cookie', $session); // remove session so that we don't bypass our test
        $this->assertNull($page->session->get('bootpress/cookie')); // make sure session is gone
        ...
        $this->assertEquals(base64_encode($session), $page->request->cookies->get('bootpress')); // session and cookie match
        $time = time();
        ...
        $session = $page->session->get('bootpress/cookie');
        list($new_id, $new_series, $new_token) = explode(' ', $session);
    ...
    public function testOnlineMethod()
    {
        $auth = new Auth();
        $this->assertEquals(1, $auth->online());
    }
    ...
    public function testGroupMethods()
    {
        $auth = new Auth();
        $auth->login(1);
        $this->assertFalse($auth->inGroup('bloggers'));
        $auth->addToGroup(1, 'bloggers');
        $this->assertTrue($auth->inGroup('bloggers'));
        $auth->addToGroup(1, 'subscribers');
        $this->assertTrue($auth->inGroup(array('bloggers', 'subscribers')));
        $auth->addToGroup(1, ''); // an empty group
        $this->assertEquals(array('bloggers', 'subscribers'), $auth->getUsersGroups(1));
        $this->assertFalse($auth->inGroup(array('bloggers', 'subscribers', ''))); // empty group not saved
        $this->assertEquals(array(1), $auth->getGroupsUsers('bloggers'));
        $this->assertEquals(array(
            'bloggers' => array(1),
            'subscribers' => array(1),
        ), $auth->getGroupsUsers(array('bloggers', 'subscribers')));
        $auth->removeFromGroup(1, 'subscribers');
        $this->assertEquals(array(
            'bloggers' => array(1),
            'subscribers' => array(),
        ), $auth->getGroupsUsers(array('bloggers', 'subscribers')));
        $this->assertEquals(array(), $auth->getGroupsUsers('subscribers'));
        $this->assertFalse($auth->inGroup(array('bloggers', 'subscribers')));
        $this->assertTrue($auth->inGroup(array('bloggers', 'subscribers'), 'any'));
    }
