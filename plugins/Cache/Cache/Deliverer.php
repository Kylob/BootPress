...
class CacheDeliverer extends CacheURLs {

  public function __construct ($file) {
    $dot = strrpos($file, '.');
    $extension = substr($file, $dot + 1);
    $filepath = substr($file, 0, $dot);
    switch ($extension) {
    ...
    if (!$this->cached($file)) { // then pull the actual file
      ...
      list($paths, $ids, $updated) = $this->uri_path($filepath);
      if (empty($paths)) {
        header( "HTTP/1.1 404 Not Found" );
        exit;
      }
      ...
      if ($image) {
        ...
        $cache = '';
        if ($type == 'css') {
          foreach ($paths as $tiny => $uri) $cache .= $this->css_get_contents($uri, $file);
        } else {
  ...
  private function css_get_contents ($file, $path) {
    ...
    foreach ($links as $relative => $uri) {
      if (isset($uris[$uri])) {
        $links[$relative] = $relative_path . $uris[$uri] . substr($uri, strrpos($uri, '.'));
      } else {
