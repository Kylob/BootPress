    ...
    $.fn.blueimpFileUploader = function(options) {
        ...
        $(this).fileupload({
            ...
            if (data.files.length != 1) {
                bootbox.alert({size:'large', backdrop:false, message:'Only 1 file may be submitted at a time'});
            } else if (settings.accept != '' && !new RegExp('\.(' + settings.accept + ')$', 'i').test(data.files[0].name)) {
                bootbox.alert({size:'large', backdrop:false, message:data.files[0].name + ' does not have a file extension of the type: ' + settings.accept.split('|').join(', ')});
            } else if (settings.size > 0 && data.files[0].size > settings.size) {
                bootbox.alert({size:'large', backdrop:false, message:data.files[0].name + ' is bigger than ' + (settings.size / 1048576) + ' MB'});
            } else if (settings.limit > 0 && $('div.' + field + 'Upload').length >= settings.limit) {
                bootbox.alert({size:'large', backdrop:false, message:'You may only upload ' + settings.limit + ' files'});
            } else if ($('#' + id(data)).length > 0) {
                bootbox.alert({size:'large', backdrop:false, message:data.files[0].name + ' has already been submitted'});
            } else {
                var status = $('<div/>').prop('id', id(data)).addClass('alert alert-warning ' + field + 'Upload').css({
                    margin: '10px 0 0',
                    padding: '8px'
                });
            ...
            bootbox.alert({size:'large', backdrop:false, message:'Error: ' + data.errorThrown + "\n\n" + data.textStatus});
            $('#' + id(data)).remove();
        }).on('fileuploaddone', function(e, data) {
            if (data.result.success) {
                $('#' + id(data)).removeClass('alert-warning alert-info').addClass('alert-success').html(data.result.success).css({cursor:'move'});
            } else {
