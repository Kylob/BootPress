...
class Admin_files extends CI_Driver {
  ...
  public function filter ($file, $slashes=false, $capitals=false) {
    if ($capitals === false) $file = strtolower($file);
    $file = str_replace(array('\\', '_'), array('/', '-'), $file);
    $file = preg_replace('/[^0-9a-z.\-\/]/i', '-', $file); // alphanumeric . - /
  ...
  private function manage ($path, $files, $main=array(), $form=false, $recursive=false) {
    ...
    if ($oldname = $ci->input->post('oldname')) {
      ...
      $newname = $this->filter($ci->input->post('newname'), $recursive, ($type == '.php'));
    ...
    if ($form) {
      ...
      if ($form->submitted() && empty($form->errors)) {
        if (!empty($ext['files']) && !empty($form->vars['file'])) {
          $file = $this->filter($form->vars['file'], $recursive, (substr($form->vars['file'], -4) == '.php'));
        ...
        if (!empty($form->vars['upload'])) {
          ...
          foreach ($form->vars['upload'] as $uploaded => $file) {
            $file = $this->filter($dir . $file, $recursive, (substr($file, -4) == '.php'));
